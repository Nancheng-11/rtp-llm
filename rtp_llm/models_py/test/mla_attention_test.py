...
from rtp_llm.models_py.modules.fmha import MlaFlashInferDecodeImpl, MlaFlashInferPrefillImpl
...
class MLATest(TestCase):
    ...
    def _run_mla_test(self, num_tokens: int, hidden_size: int, page_size: int):
        ...
        kvcache_block_id = torch.zeros(
            [batch_size, max(seq_page_sizes)], dtype=torch.int32, device=torch.device("cpu")
        )
        bias = 1
        ...
        sequence_lengths_t = torch.tensor(sequence_lengths_mius_1, dtype=torch.int32, device=torch.device("cpu"))
        prefix_lengths_t = torch.zeros(
            len(sequence_lengths_t) - len(sequence_lengths) + 1, dtype=torch.int32, device=torch.device("cpu"),
        )

        attn_inputs: PyAttentionInputs = PyAttentionInputs()
        attn_inputs.is_prefill = True
        attn_inputs.prefix_lengths = prefix_lengths_t
        attn_inputs.sequence_lengths = torch.tensor([], dtype=torch.int32, device=torch.device("cpu"))
        ...
        fmha_impl = MlaFlashInferPrefillImpl(self.config, attn_inputs, layer_weights, create_cos_sin_cache())
        ...
        self.assertTrue(
            torch.allclose(out, out_ref, atol=1, rtol=1)
        )
